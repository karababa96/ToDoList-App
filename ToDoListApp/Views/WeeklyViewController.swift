...
class WeeklyViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func getData() {
        ...
        dataBase.collection("WeeklyNotes").addSnapshotListener { (snapshot, error) in
            if error != nil {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            dataBase.collection("WeeklyNotes").document(self.documentIDArray[indexPath.row]).delete { (error) in
                if error != nil {
                    print(error?.localizedDescription)
                }else {
